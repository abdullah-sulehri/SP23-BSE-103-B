...
server.get("/admin/ordersView", async (req, res) => {
  try {
    // Fetch all orders from the database, sorted by date in descending order
    const orders = await Order.find().sort({ orderDate: -1 });

    // Render the EJS template and pass the orders data
